...
def test_run_ingests_both_platforms_into_one_snapshot(tmp_path, fetched_at):
    ...
    per_platform = connection.execute(
        "SELECT platform, COUNT(*) FROM price_snapshots GROUP BY platform"
    ).fetchall()
    assert dict(per_platform) == {"kalshi": 4, "polymarket": 2}
    # Every row of one run shares fetched_at: a run is one coherent snapshot.
    timestamps = connection.execute(
        "SELECT DISTINCT fetched_at FROM price_snapshots"
    ).fetchall()
...
def test_run_groups_multi_outcome_event_for_phase2(tmp_path, fetched_at):
    ...
    rows = connection.execute(
        "SELECT event_id, COUNT(*) FROM price_snapshots"
        " WHERE mutually_exclusive = 1 GROUP BY event_id"
    ).fetchall()
